...
- The use of CHAD and Grothendieck logical relations provides a mathematically robust foundation for verifying slicing correctness.

- The high-order language design validates the framework’s adaptability to diverse languages, enhancing practical relevance.

- It is nice to see the linkage between stable functions (from domain theory) and smooth functions (from differential geometry).

### Weak Points

- While the introduction is accessible, the subsequent sections are highly technical and challenging for non-experts to follow. Key intuitions are often obscured by dense formalism.

- The paper lacks empirical validation or comparisons with existing slicing tools, making it difficult to assess real-world applicability.
...
After the introduction (Section 1), Section 2 develops an analogy between Berry’s notion of stability in denotational semantics and the notion of smoothness in calculus. The section reviews existing results, discusses the analogy informally, and culminates in a conjecture (Conjecture 2.15, line 498), which is intended to serve as the true witness of this analogy.

Section 3 then turns to the free coproduct completion $\mathrm{Fam}(\mathcal{C})$ of a category $\mathcal{C}$ (Def. 3.1, lines 524 and 573) and examines its application to specific categories: the category $\mathbf{LatGal}$ of bounded lattices and Galois connections, and the two categories of semilattices, $\mathbf{MeetSLat}$ and $\mathbf{JoinSLat}$ (lines 549–559).
In particular, drawing on a result of Lucatelli Nunes et al. (Thm. 3.6, line 602), the authors establish that $\mathrm{Fam}(\mathbf{MeetSLat})$ is cartesian closed (Cor. 3.7, line 614), as is $\mathrm{Fam}(\mathbf{MeetSLat} \times \mathbf{JoinSLat}^{op})$ (Cor. 3.13).

Unfortunately, this result does not extend to $\mathrm{Fam}(\mathbf{LatGal})$, since the latter category lacks infinite products. However, there exists a faithful functor H from $\mathrm{Fam}(\mathbf{LatGal})$ to $\mathrm{Fam}(\mathbf{MeetSLat} \times \mathbf{JoinSLat}^{op})$ which will be used to define a semantics in $\mathrm{Fam}(\mathbf{LatGal})$ for functional programs with first-order types, drawing on results of Fiore and Simpson (Thm. 5.1, line 1042, and Thm. 5.2, line 1057).
...
2. It offers several intriguing indications of analogies with recent work in differentiable programming, such as CHAD and differential reverse categories.

3. It proposes an interesting conjecture (Conjecture 2.15), aiming to relate Berry’s notion of stability to smoothness in analysis.
...
2. The title and abstract of the paper are misleading, as both refer to automatic differentiation.
This is a technique based on program transformation (and/or operator overloading) for computing derivatives, Jacobians, etc., of numerical programs, by exploiting their syntactical structure.
The paper, however, does not address such transformations.
Instead, it draws (informal) analogies between the categorical structures of $\mathrm{Fam}(\mathbf{MeetSLat} \times \mathbf{JoinSLat}^{op})$ and those in CHAD and reverse differential categories, without presenting precise technical results in this direction. This is a good point but I would not present it as the major contribution of the paper.

3. The programming language considered is standard, and the core technical result of the paper is giving a denotational models based on Galois connections. While this result is both interesting and novel, it is also rather technical. I do not see how such a contribution shows that Galois slicing is a "generalised form of automatic differentiation". I would rather say that this contribution is an application of a known categorical construction to build a denotational model of a standard programming language. The paper might therefore be better suited to a more domain-specific conference.

4. One of the main digressions appears in Section 2, where an analogy between Berry’s stability and smoothness is discussed.
This is an appealing intuition, and I was enthusiastic to read this part.
Unfortunately, the discussion remains purely informal, culminating only in a conjecture as a technical counterpart.
Moreover, this analogy is not new: it is for example the starting point for differential linear logic, where the trace of a stable function is expressed via a Taylor expansion (see Ehrhard, *An introduction to differential linear logic: proof-nets, models and antiderivatives*, MSCS 2018).
...
- Line 410 cites Taylor [1999] for a characterization of stability in terms of Galois connections. However, this is a book of more than 600 pages.
