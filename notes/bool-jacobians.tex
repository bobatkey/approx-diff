...
position $j$ in the domain. Matrix composition then matches reachability:
\begin{salign*}
   (M_g \comp M_f)[i,k] = \bigjoin_j M_g[i,j] \meet M_f[j,k]
\end{salign*}
\noindent is the $(\join, \meet)$-semiring matrix product, which evaluates to $\top$ iff there is some
intermediate position $j$ with both $M_g[i,j] = \top$ and $M_f[j,k] = \top$, i.e.~iff there is a chain $k \to
j \to i$ witnessing that $i$ depends on $k$ via $j$.
