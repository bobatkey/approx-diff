...
\item For every $\Pred(X)$, the structure of a Heyting algebra with universal quantification, i.e.~$\Pred(X)$
   is closed under top element $\top$, meets $\meet$, joins $\join$, residual $\Rightarrow$, and unary
   connective $\forall$, satisfying the usual laws;
