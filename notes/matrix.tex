...
\cdot v) = a \cdot f(v)$), which is the usual notion of linearity extended to the semiring setting. Every such
$f$ satisfies
\begin{salign*}
   f(v) = \sum_i v_i \cdot f(e_i) \tag{basis decomposition of linear maps}
\end{salign*}
\noindent so $f$ is determined by the $m$ vectors $f(e_1), \ldots, f(e_m) \in S^n$. These form the columns
of an $n \times m$ matrix $M_f$, the \emph{matrix representation} of $f$; conversely every $n \times m$
matrix is $M_f$ for a unique such $f$.

\subsubsection{Bounded distributive lattice}
\label{sec:matrix:additional-structure:lattice}
