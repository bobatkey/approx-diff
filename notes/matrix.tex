...
and $n$, the biproduct $m \biprod n$ can be constructed as the sum $m + n$ with block-matrix projections
and injections
\begin{salign*}
   p_1 &= \begin{bmatrix} I_m & 0 \end{bmatrix} : m+n \to m, &
   p_2 &= \begin{bmatrix} 0 & I_n \end{bmatrix} : m+n \to n, \\
   \iota_1 &= \begin{bmatrix} I_m \\ 0 \end{bmatrix} : m \to m+n, &
   \iota_2 &= \begin{bmatrix} 0 \\ I_n \end{bmatrix} : n \to m+n,
\end{salign*}
\noindent where $I_m, I_n$ denote identity matrices and the $0$s are zero matrices of compatible size.
Pairings and copairings are vertical and horizontal concatenation of matrices: for $M : k \to m$ and $N : k
\to n$, the pairing $\prodM{M}{N} : k \to m+n$ stacks $M$ above $N$; for $M : m \to k$ and $N : n \to k$,
the copairing $\coprodM{M}{N} : m+n \to k$ places $M$ and $N$ side by side. $\Mat(S)$ is therefore a
