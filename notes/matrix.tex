...
A \emph{length-$n$ vector} over $S$ is an element $v \in S^n$, with $i$-th component $v_i$. The \emph{standard
basis vector} $e_i \in S^n$ at index $i$ is the Kronecker $\delta$,
\begin{salign*}
   (e_i)_j =
      \begin{cases}
         1 & \text{if }i = j; \\
...
Write $\sum_i a_i$ for the iterated semiring sum of $a_1, \ldots, a_n \in S$, and $u \cdot v = \sum_i u_i
\cdot v_i$ for the \emph{dot product} of two length-$n$ vectors. Iterated sum lifts to vectors $w_1, \ldots,
w_m \in S^n$ pointwise:
\begin{salign*}
   \left(\bigoplus_{i} w_i\right)_{\!j} = \sum_i (w_i)_j.
\end{salign*}
...
A matrix $M : m \to n$ is an element $M \in S^{n \times m}$ with entry $M_{ij}$ at row $i$ and column $j$.
Equivalently, a matrix is an $m$-indexed family of length-$n$ vectors (its columns). Composition of $N : n
\to k$ and $M : m \to n$ is matrix multiplication,
\begin{salign*}
