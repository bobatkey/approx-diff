...
If additionally $\cdot$ is the meet of a top element $1$ and distributes over $+$ — i.e.\ $(S, +, 0, \cdot, 1,
\le)$ is a bounded distributive lattice, then we have a \emph{disjointness} relation $a \disj b$ iff $a \cdot
b \le 0$, with pointwise lift to vectors. If $S$ is furthermore a \emph{Boolean algebra}, equipped with a
...
\end{salign*}
\noindent which evaluates to $\top$ iff there is some intermediate $j$ with $k\, R_M\, j$ and $j\, R_N\, i$,
or equivalently iff $i$ is reachable from $k$ via $R_M$ then $R_N$. The dagger $M \mapsto M^{\!\top}$
corresponds to the relational \emph{converse}: $i\, R_{M^{\!\top}}\, j \iff j\, R_M\, i$, reversing the
