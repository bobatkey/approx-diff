...
(w_i)_j$), the iterated sum $\bigoplus_i w_i \in S^n$ is the row-sum of $W$:
\begin{salign*}
...
The iterated sum notation $\bigoplus$ from \secref{matrix:vectors} aligns with the biproduct $\biprod$: a
vector $w \in S^n$ is the same as a morphism $w : 1 \to n$ in $\Mat(S)$, and for $w_1, \ldots, w_m \in S^n$
the iterated sum $\bigoplus_i w_i$ is their $\CMon$ addition in $\Mat(S)(1, n)$. Equivalently, $\bigoplus_i
w_i = W \comp \diag_m$, where $W : m \to n$ has the $w_i$ as columns and $\diag_m : 1 \to m$ is the
diagonal into the biproduct $1 \biprod \cdots \biprod 1$.
...
= b$, $\sum_i$ becomes iterated join (and $\bigoplus_i$ its pointwise lift), and basis decomposition
extends to any $\join$-preserving, scale-linear $f : S^m \to S^n$:
\begin{salign*}
