...
We note that if a category $C$ is \emph{$\CMon$-enriched}, we have that:
\begin{enumerate}
\item Every hom-object $\Hom{C}{A}{B}$ is a commutative monoid; the unit $\zero_{A,B}$ is a zero morphism and
the binary operation $+_{A,B}$ is addition of morphisms.
...
\begin{enumerate}
\item unit $0$ is the constant homomorphism sending every element of $A$ to $\varepsilon_B$;
\item addition of morphisms $+$ is defined pointwise so that $+_{A}$ so that $(f + g)(a) = f(a) + g(a)$.
\end{enumerate}
