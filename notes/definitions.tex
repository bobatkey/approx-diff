...
A \emph{commutative monoid} $A = (A, \varepsilon, +)$ is a set equipped with distinguished element
$\varepsilon \in A$ called the \emph{unit} and associative binary operation $+$ satisfying $\varepsilon + x =
x$ and $x + \varepsilon = x$ for any $x \in A$. A commutative monoid homomorphism from $A$ to $B$ is any
function $f: A \to B$ which preserves $\varepsilon$ and $+$.

\subsection{Category of commutative monoids}

$\CMon$ is the category whose objects are the commutative monoids $A$ and morphisms $A \to B$ are the
commutative monoid homomorphisms. $\CMon$ is symmetric closed monoidal, with the trivial one-element monoid
$\One$ as terminal object and monoidal product $A \tensor B$ given by the direct product of $A$ and $B$.
$\CMon$ is complete and cocomplete, inheriting all limits and colimits from $\Set$.

\subsection{$\CMon$-enriched category}

A category $C$ is \emph{$\CMon$-enriched} if:
\begin{enumerate}
\item every hom-object $\Hom{C}{A}{B}$ is a commutative monoid with a
\emph{zero} morphism $\zero_{A,B}$ as unit and addition of morphisms $+_{A,B}$ as the monoid operation;
\item composition is a family of morphisms $\Hom{C}{B}{C} \tensor \Hom{C}{A}{B} \to \Hom{C}{A}{C}$ in $\CMon$,
i.e.~satisfies
...
monoid, with:
\begin{enumerate}
\item unit given by the constant homomorphism that sends every element of $A$ to $\varepsilon_B$;
