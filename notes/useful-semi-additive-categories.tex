...
Suppose $X$ and $Y$ are bounded lattices. A \emph{conjugate pair} $f \mathrel{{\sim}} g : X \to Y$ is a pair of
...
$\id_X \mathrel{{\sim}} \id_X$ is a conjugate pair, so the following is a category.

\begin{definition}[Category $\HeytConj$]
Define the category $\HeytConj$ which has:
\begin{itemize}
\item as objects all Heyting algebras;
\item as morphisms from $X$ to $Y$ all conjugate pairs $f \mathrel{{\sim}} g : X \to Y$.
\end{itemize}
\end{definition}
