...
conjugate pair becomes an alternative presentation of a Galois connection: $(f, g)$ is a conjugate pair iff
$f \adj (\neg \comp g)$ (equivalently, $(\neg \comp f) \adj g$) is a Galois connection. So over Heyting algebras a conjugate pair is
``a Galois connection with a pseudocomplement inserted on one side''; this is also why both components are
forced to preserve joins (composition with $\neg$ swaps the meet-preserving right adjoint into a
join-preserving map).
