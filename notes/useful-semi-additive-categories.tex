...
going back to Jónsson and Tarski. Whereas a Galois connection links two directions by the order $\le$, a
conjugate pair links them by the \emph{disjointness} relation
$a \disj b \iff a \meet b \le \bot$.
...
\noindent Both components of a conjugate pair are automatically $\join$-preserving (and send $\bot$ to
$\bot$), provided the underlying lattice has enough structure for disjointness to determine order --- concretely,
it suffices that each $X$ be a \emph{Heyting algebra} (a distributive lattice with an implication
operation), because then $a \le b \iff \neg b \disj a$ via the pseudocomplement $\neg b$. Conjugate pairs
compose componentwise (with the second components composed in the opposite order), and the identity
...
\noindent Like $\LatGal$, $\HeytConj$ is $\CMon$-enriched (in fact $\SemiLat$-enriched): the zero morphism
$0_{X,Y}$ is the constant-$\bot$ pair, and addition of morphisms is componentwise pointwise $\join$. Unlike
$\LatGal$, in $\HeytConj$ both components preserve joins, so the category carries a dagger. We return to this
point in \secref{boolean-jacobians:embeddings}.
