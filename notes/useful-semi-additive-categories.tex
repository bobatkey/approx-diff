...
\noindent Like $\LatGal$, $\HeytConj$ is $\SemiLat$-enriched, with zero morphism $0_{X,Y}$ given by the
constant-$\bot$ pair and addition of morphisms given componentwise by pointwise $\join$. Unlike $\LatGal$,
in $\HeytConj$ both components preserve joins, so the category carries a dagger. We return to this point in
