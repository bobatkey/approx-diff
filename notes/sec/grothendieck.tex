...
\item The family of morphisms $\eval_{X,Y}: [X, Y] \times X \to Y$ in $\Fam(C)$ where $\eval_{X,Y} = (((f, g),
i) \mapsto f(i): \Fam(C)(X,Y) \times I \to J, \{[{\eval_{\prod}}_i, g_i] \mid ((f, g), i) \in \Fam(C)(X,Y)
\times I\}: X \to \reindex{Y}{f})$ (using $[-,-]$ to denote coproduct of morphisms).
\end{enumerate}
