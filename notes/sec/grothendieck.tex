...
\item $g'(i) = \lambda_{\prod_{J}}(j \mapsto g_{i, j} \comp \inj_{X_i})$ where $\inj_{X_i}: X_i \to X_i \biprod
Y_j$ for every $j \in J$
\end{itemize}
\item The family of morphisms $\eval_{X,Y} = (f', g'): [X, Y] \times X \to Y$ in $\Fam(C)$ with
\begin{itemize}
\item $f'((f, g), i) = f(i)$
\item $g'((f, g), i) = \coprodM{{\eval_{\prod}}_i}{g_i}: \prod_{I}\reindex{Y}{f} \biprod X_i \to Y_{f(i)}$
\end{itemize}
