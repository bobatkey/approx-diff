...
\item The family of morphisms $\eval_{X,Y} = (f': \Fam(C)(X,Y) \times I \to J, g': [X, Y] \times X \to
\reindex{Y}{f}): [X, Y] \times X \to Y$ in $\Fam(C)$ with
\begin{itemize}
\item $f'((f, g), i) = f(i)$
\item $g' = \{[{\eval_{\prod}}_i, g_i] \mid ((f, g), i) \in \Fam(C)(X,Y) \times I\}$, using $[-,-]$ to denote
