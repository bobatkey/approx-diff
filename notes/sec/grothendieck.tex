...
\idx{Y} \to \idx{Z}$. For $\faml{\lambda}_{X,Y,Z}$, define the following family of functions sending any
$(f,g): X \to [Y,Z]$ to $\lambda^{-1}_{X,Y,Z}(f,g) = (f^\dagger, g^\dagger): X \times Y \to Z$ with:
\begin{itemize}
\item $f^\dagger(x,y) = f'(y)$ where $f(x) = (f',g')$
\item $g^\dagger_{x,y} = g'_y \comp \proj_{\faml{Y}_y}$ where $f(x) = (f',g')$ and
$\proj_{\faml{Y}_y}: \faml{X}_x \biprod \faml{Y}_y \to \faml{Y}_y$
\end{itemize}
