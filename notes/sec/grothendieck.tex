...
\noindent In $\Fam(C)$ the objects $X$ are thus pairs of a set $\idx{X}$ and indexed family $\faml{X}$ in
$\Fam(\idx{X},C)$ and morphisms $f: X \to Y$ are functions $\idx{f}: \idx{X} \to \idx{Y}$ paired with a
morphism $\faml{f}: X \to \reindex{Y}{f}$ in $\Fam(\idx{X},C)$.
...
\begin{enumerate}
\item $[X, Y]$ to be the object $(\Fam(C)(X,Y),[\idx{X}, \idx{Y}])$ in $\Fam(C)$ where $[\idx{X},
\idx{Y}]$ denotes the indexed family which maps every $f: X \to Y$ in $\Fam(C)$ to
$\prod_{I}\reindex{Y}{\idx{f}}$.
\item The family of isomorphisms $\lambda_{X,Y,Z}$ (natural in $X$) sending any morphism $f: X \times Y \to Z$
to $g: X \to [Y, Z]$ with:
\begin{itemize}
\item $\idx{g}(i) = (j \mapsto \idx{f}(i, j): J \to K, j \mapsto \faml{f}_{i,j} \comp \inj_{Y_j}: Y \to
\reindex{Z}{j \mapsto \idx{f}(i, j)})$ where $\inj_{Y_j}: Y_j \to X_i \biprod Y_j$
