...
\begin{enumerate}
\item $[X, Y]$ to be the object $(\Fam(C)(X,Y),[\idxf{X}, \idxf{Y}])$ in $\Fam(C)$ where $[\idxf{X},
\idxf{Y}]$ denotes the indexed family which maps every $f: X \to Y$ in $\Fam(C)$ to
$\prod_{I}\reindex{Y}{\idxf{f}}$.
