...
\begin{enumerate}
\item Suppose $(x, y) \in \idx{X} \times \idx{Y}$. The diagram on the left below commutes by the universal property
of the set-indexed product; the commutativity of the diagram on the right is trivial.
...
\inj_{\faml{X}_x}){,}\,\id_{\faml{Y}_y})"] \arrow[r, "\coprodM{\faml{f}_{x,y}\,\comp\,\inj_{\faml{X}_x}}{\faml{f}_{x,y}\,\comp\,\inj_{\faml{Y}_y}}"'] & \faml{Z}_{\idx{f}(x,y)}
\end{tikzcd}
\hspace{1cm}
\begin{tikzcd}
