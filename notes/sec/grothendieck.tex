...
\begin{itemize}
\item $f'(f, x) = \idx{f}(x)$
\item $g'_{f, x} = \coprodM{{\eval_{\prod}}_x}{\faml{f}_x}:
...
\end{itemize}
\noindent To show $f' = f : \idx{X} \to \Fam(C)(Y,Z)$, suppose $x \in \idx{X}$. To show $f'(x) = f(x): Y \to
Z$, suppose $y \in \idx{Y}$.
\begin{enumerate}
\item $(f^\dagger \comp (x, -))(y) = f^\dagger(x,y) = \idx{f(x)}(y)$.
\item $(\reindex{g^\dagger}{(x,-)} \comp \inj_{\faml{Y}})_y = \reindex{g^\dagger}{(x,-)}_y \comp
{\inj_{\faml{Y}}}_y = g^\dagger_{x,y} \comp {\inj_{\faml{Y}}}_y = \coprodM{{\eval_{\prod}}_y \comp
g_x}{\faml{f}_y} \comp {\inj_{\faml{Y}}}_y = g_x$.
\end{enumerate}

To show $g' = g$, suppose $x \in \idx{X}$. To show $g'_x = g_x$:
\begin{align*}
\small
g'_x
&=
...
&=
\lambda_{\prod_{\idx{Y}}}(\{\coprodM{{\eval_{\prod}}_y \comp g_x}{\faml{f(x)}_y}\}_{y \in \idx{Y}})
\end{align*}
