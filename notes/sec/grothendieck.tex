...
\begin{proof}
Suppose $C$ locally small with binary biproducts given by $\biprod$. Suppose $X = (I, X)$ and $Y = (I, Y)$ are
objects in $\Fam(C)$. Define $[X, Y]$ to be the object $(K,Z)$ in $\Fam(C)$ where $K$ is $\Fam(C)(X,Y)$ and
$Z$ is the $K$-indexed family which maps every $(f: I \to J, g: X \to \reindex{Y}{f})$ in $K$ to $\prod_{i \in
I}\reindex{Y}{f}_i$. Then define the following families of functions:
