...
$\faml{g} = \faml{f} \comp (x,-) \comp \inj_{\faml{Y}}: \faml{Y} \to \reindex{\faml{Z}}{\idx{g}}$ where $\inj_{\faml{Y}}:
\faml{Y} \to \const_{\idx{Y}}(\faml{X}_x) \biprod \faml{Y}$
\item $\faml{\lambda}_{X,Y,Z}(f)(x) = \lambda_{\prod_{\idx{Y}}}(\faml{f} \comp (x,-) \comp \inj_{\faml{X}_x}:
...
\begin{tikzcd}
   ...
   "\coprodM{(\eval_{\prod_{\idx{Y}}})_y}{\faml{f}_y}"] \\
   \faml{X}_x \biprod \faml{Y}_y \arrow[u, "\lambda_{\prod_{\idx{Y}}}(\faml{f}\,\comp\,{{(x,-)}}\,\comp\,
