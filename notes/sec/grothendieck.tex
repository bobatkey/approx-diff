...
\begin{enumerate}
\item $[X, Y]$ to be the object $(\Fam(C)(X,Y),[X, Y])$ in $\Fam(C)$ where (overloading notation) $[X,Y]$ is
the indexed family which maps every $(f: I \to J, g: X \to \reindex{Y}{f})$ in $\Fam(C)(X,Y)$ to $\prod_{i \in
I}\reindex{Y}{f}_i$.
