...
\item $\faml{\lambda}_{X,Y,Z}(f)(i) = \lambda_{\prod_{\faml{J}}}(j \mapsto \faml{f}_{i, j} \comp
...
\item $\faml{\eval}_{X,Y}((f, g), i) = \coprodM{{\eval_{\prod}}_i}{g_i}: \prod_{\idx{X}}\reindex{\faml{Y}}{f}
\biprod \faml{X}_i \to \faml{Y}_{f(i)}$
\end{itemize}
...
\hspace{1cm}
\begin{tikzcd}
   {{[Y, Z]}} \biprod Y_y \arrow[dr, "\eval_{Y,Z}"] \\
   X_x \biprod Y_y \arrow[u, "\lambda_{X,Y,Z}(f) \biprod \id_{Y_y}"] \arrow[r, "f"'] & Z_{f(x,y)}
\end{tikzcd}
\end{center}
