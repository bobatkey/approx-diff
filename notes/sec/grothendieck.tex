...
\begin{itemize}
\item $\idx{\lambda}_{X,Y,Z}(f)(x) = (\idx{f} \comp (x, -), \reindex{\faml{f}}{(x,-)} \comp \inj_{\faml{Y}}):
...
\begin{enumerate}
\item $\idx{\lambda}_{X,Y,Z}$ is an isomorphism because $\idx{X} \times \idx{Y} \to \idx{Z} \iso \idx{X} \to
\idx{Y} \to \idx{Z}$. For $\faml{\lambda}_{X,Y,Z}$, define the following family of functions sending any $f: X
\to [Y,Z]$ to $\lambda^{-1}_{X,Y,Z}(f): X \times Y \to Z$ with:
\begin{itemize}
\item $\idx{\lambda^{-1}_{X,Y,Z}}(f)(x,y) = \idx{\idx{f}(x)}(y)$
\item $\faml{\lambda^{-1}_{X,Y,Z}}(f)(x,y) = \faml{g}_y \comp \proj_{\faml{Y}}$ where $\idx{f}(x) = g$
\end{itemize}
