...
\begin{enumerate}
\item $\idx{\lambda}_{X,Y,Z}$ is an isomorphism by the isomorphism of $\idx{X} \times \idx{Y} \to \idx{Z}$ and
$\idx{X} \to \idx{Y} \to \idx{Z}$. For $\faml{\lambda}_{X,Y,Z}$, define the family of functions
$\lambda^{-1}_{X,Y,Z}$ sending any $f: X \to [Y,Z]$ to $\lambda^{-1}_{X,Y,Z}(f): X \times Y \to Z$ with:
\begin{itemize}
