...
\item $g^\dagger_{x,y} = \coprodM{{\eval_{\prod}}_y \comp g_x}{g'_y}$ where $f(x) = (f',g')$
\end{itemize}

Want to show $\lambda_{X,Y,Z}(f^\dagger, g^\dagger) = (f,g)$.
\begin{enumerate}
\item Suppose $x \in \idx{X}$ with $f(x) = (f',g')$. For any $y \in \idx{Y}$ we then have:
\begin{enumerate}
\item $(f^\dagger \comp (x, -))(y) = f^\dagger(x,y) = f'(y) = f(y)$
\item $\reindex{g^\dagger}{(x,-)} \comp \inj_{\faml{Y}} = $.
\end{enumerate}
\end{enumerate}
