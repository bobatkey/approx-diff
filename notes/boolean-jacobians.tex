...
\emph{right adjoint} $f^*: \Two^n \to \Two^m$ (meet-preserving, with $f \adj f^*$); these are De Morgan
...
plays the role of the reverse-mode $\pullf{\phi}_x$. The adjoint map $f^*$ seems to be a different analysis
...
\Two^m$, since $M_f[i,j]$ is precisely the $i$-th coordinate of $f(e_j)$. Concretely,
\begin{salign*}
   f(v) = \bigjoin_{j : v_j = \top} f(e_j)
\end{salign*}
\noindent which follows from $f$ being $\join$-preserving together with the biproduct decomposition $v =
\bigjoin_j v_j \scalarmeet e_j$ (where $v_j \scalarmeet e_j$ equals $e_j$ if $v_j = \top$ and $\bot$
otherwise). Conversely, any $m$-tuple $(y_1, \ldots, y_m) \in (\Two^n)^m$ arises as $(f(e_1), \ldots,
f(e_m))$ for a unique such $f$, giving the isomorphism $\SemiLat(\Two^m, \Two^n) \cong (\Two^n)^m$. Thus
$\SemiLat$ restricted to powers of $\Two$ is equivalent to a category of Boolean matrices, with composition
given by the usual matrix product in the $(\join, \meet)$-semiring.
