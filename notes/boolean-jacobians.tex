...
Throughout let $\Two = \{\bot, \top\}$ be the two-element Boolean algebra, viewed as a join semilattice via
its join $\join$ and bottom $\bot$. Write $\SemiLat$ for the category of (join) semilattices and
$\join$-preserving maps; it is $\CMon$-enriched and has biproducts. (Dually, $\SemiLat$ may be read as the
category of meet semilattices and $\meet$-preserving maps; the two readings give isomorphic categories and
we move freely between them where helpful.)

\subsubsection{Overview}
\label{sec:boolean-jacobians:overview}

We study a ``Bool-linear'' subcategory of $\SemiLat$ whose objects are the finite powers $\Two^n$ of the
two-element join semilattice. Given a $\join$-preserving map $f$ in this category, we consider two
canonical companions going in the opposite direction: the \emph{transpose} $f^{\!\top}$ (also
$\join$-preserving) and the \emph{right adjoint} $f^*$ (meet-preserving, with $f \adj f^*$). The pair
$(f, f^{\!\top})$ is the Boolean analogue of the forward/reverse-mode Jacobian pair in AD, related by the
dagger operation. The right adjoint $f^*$ is a separate, order-theoretic construction with no AD analogue;
in the Boolean setting it is related to $f^{\!\top}$ by De Morgan duality.

\begin{itemize}
\item Objects are finite Boolean vector spaces $\Two^n$, arising as iterated biproducts
(\secref{boolean-jacobians:vectors}).
\item Morphisms $\Two^m \to \Two^n$ are $n \times m$ Boolean matrices, equivalently, $\join$-preserving maps
determined by their values on basis vectors; composition is matrix multiplication in the
$(\join, \meet)$-semiring (\secref{boolean-jacobians:matrices}).
\item Every morphism has a transpose, making this a dagger semi-additive category
(\secref{boolean-jacobians:transpose}).
\item The transpose embeds the category contravariantly into $\HeytConj$ as conjugate pairs; the right
adjoint embeds it contravariantly into $\LatGal$ as Galois connections; these two embeddings are De Morgan
duals of each other (\secref{boolean-jacobians:embeddings}, \secref{boolean-jacobians:de-morgan}).
\item Under the analogy between this setting and $\FinVect_\RR$, the transpose $f^{\!\top}$ corresponds to
the forward-mode Jacobian $\pushf{\phi}_x$ and the primitive $f$ plays the role of the reverse-mode
$\pullf{\phi}_x$; the Galois connection's adjunction structure is a genuinely different analysis with no
direct real-AD analogue (\secref{boolean-jacobians:ad}).
\end{itemize}

\subsubsection{Boolean vectors as iterated biproducts}
\label{sec:boolean-jacobians:vectors}

Define $\Two^n$ inductively as the $n$-fold biproduct of $\Two$ in $\SemiLat$: $\Two^0$ is the zero object
and $\Two^{n+1} = \Two \biprod \Two^n$. Its carrier is the set of length-$n$ vectors of Boolean values, with
...
morphism), each entry $M_f[i,j]$ can be identified with an element of $\Two$, and $f$ corresponds to a
genuine $n \times m$ Boolean matrix.

Conversely, \emph{every} morphism $f: \Two^m \to \Two^n$ in $\SemiLat$ is determined by its values on the
``standard basis vectors'' $e_j = \biinj_j(\top): \Two^m$: concretely,
\begin{salign*}
   f(v) = \bigjoin_{j : v_j = \top} f(e_j)
\end{salign*}
\noindent This is a consequence of $f$ being $\join$-preserving together with the biproduct decomposition
$v = \bigjoin_j v_j \scalarmeet e_j$ (where $v_j \scalarmeet e_j$ is the ``scaled basis vector'' that equals
$e_j$ if $v_j = \top$ and $\bot$ otherwise). Thus $\SemiLat$ restricted to powers of $\Two$ is equivalent
to a category of Boolean matrices, with composition given by the usual matrix product in the
$(\join, \meet)$-semiring.

Concretely, $M_f$ is exactly the \emph{dependency relation} between the two index sets: $M_f[i,j] = \top$
iff position $i$ in the codomain is ``live'' in $f(e_j)$, i.e.\ iff position $i$ in the codomain depends on
position $j$ in the domain. Matrix composition then matches reachability: $(M_g \comp M_f)[i,k] = \top$ iff
there is some intermediate position $j$ with $M_g[i,j] = \top$ and $M_f[j,k] = \top$, which is the usual
``$\join$ over chains of $\meet$s'' semiring product.
...
The two embeddings then pair $f$ with a companion going the forward direction, in two different ways:
...
\item \emph{Galois connection.} The right adjoint $f^*: \Two^n \to \Two^m$ (meet-preserving) pairs with $f$
to form a Galois connection in $\LatGal$ (\secref{categories-with-biproducts:latgal}), with adjunction
\begin{salign*}
   y \le f^*(x) \iff f(y) \le x
\end{salign*}
\noindent Concretely, $f^*(x)_i = \bigmeet_j (M_f[i,j] \to x_j)$, where $a \to b = \neg a \join b$ is
Boolean implication. In the target arrow $\Two^n \to \Two^m$, $f$ is the left adjoint and $f^*$ the right
adjoint.
\end{enumerate}
...
conjugate embedding as a derived construction: the existence of a Galois connection $(f^*, f)$ together
...
played by the Bool-linear subcategory of $\SemiLat$, with biproducts $\Two^n$ and morphisms given by Boolean
...
Care is needed in lining up directions. In the preceding subsections, our primitive $f$ lives in
...
$f^{\!\top}: \Two^n \to \Two^m$: here $n$ indexes the inputs and $m$ indexes the outputs of that program. The primitive $f: \Two^m \to \Two^n$, going from outputs to inputs, is
the Boolean analogue of the \emph{reverse-mode} map $\pullf{\phi}_x$. Transposition is involutive, so this
is only a choice of labelling: the dagger pair $(f, f^{\!\top})$ captures exactly the same content as
...
(matrix transpose) to relate the two directions, and both directions live in the same linear / Bool-linear
category. The Galois embedding is a genuinely different analysis. The right adjoint
$f^*: \Two^n \to \Two^m$ is \emph{not} Bool-linear in the same sense: it is $\meet$-preserving rather than
$\join$-preserving, and lives in the \emph{dual} semi-additive structure where $\meet$ plays the role of
addition. Concretely, reading $f^*$ in the standard sense gives the ``sufficiency'' interpretation (``is
every input that output $i$ depends on present?''), a universally quantified condition in a different
semiring.

More subtly, it is not just $f^*$ that is foreign to AD but the entire adjunction structure. The Galois
connection pairs $f$ with $f^*$ via $f \adj f^*$, and this adjunction has no analogue in $\FinVect_\RR$: the
...
the left adjoint of a Galois connection --- and the companion map $f^*$ that role brings with it --- is a
...
\begin{center}
\begin{tabular}{ll}
 \textbf{Real AD} ($\FinVect_\RR$) & \textbf{Boolean AD} (Bool-linear subcategory of $\SemiLat$) \\
 Linear map $\pushf{\phi}_x: \RR^p \linearto \RR^q$ (forward mode) & $\join$-preserving $f^{\!\top}: \Two^n \to \Two^m$ \\
 Linear transpose $\pullf{\phi}_x: \RR^q \linearto \RR^p$ (reverse mode) & Primitive $f: \Two^m \to \Two^n$ \\
 Matrix multiplication (chain rule) & Composition in the $(\join, \meet)$-semiring \\
 Dagger on $\FinVect_\RR$ via inner product & Dagger on the Bool-linear subcategory via $\bigjoin / \meet$ \\
\end{tabular}
\end{center}
