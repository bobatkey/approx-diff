...
Logic via Coherence Spaces~\citep{girard}. A textbook description of
stable functions in the context of domain theory is given by
\citet[Chapter 12]{amadio-curien}. Here, we are not
...
\begin{example}[Functions that are not stable]
  A function that is not stable is \citeauthor{lcf77}'s Parallel OR~\citeyearpar{lcf77},
