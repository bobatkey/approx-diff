...
\begin{example}[Stability reveals intensional information]
  ...
  ORs\footnote{These clauses are shorthand for the graph of the
    function. They are not to be understood as pattern matching
    clauses in a language like Haskell.}:
  \begin{displaymath}
  ...
  minimal input is $(\bot, \bot)$.

  The function $\mathrm{shortCircuitOR}$ is also stable. For the
  input-output pair $(\mathsf{tt},\mathsf{ff}),\mathsf{tt}$, the
  minimal input that gives this input is $(\mathsf{tt},\bot)$,
  indicating that the presence of $\mathsf{ff}$ in the second argument
  was not necessary to produce this output. As with
...
\begin{example}[Functions that are not stable]
  ...
  function does not present a problem. For any particular run of the
