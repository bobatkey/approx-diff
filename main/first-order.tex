...
To interpret the syntactic type constructor $\synVar{L}$, consider the functor $\mathcal{L}$ which extends any
poset $X$ with a distinguished bottom element $\bot$ and any monotone function $f$ between posets with a
mapping from $\bot$ to $\bot$. $\mathcal{L}$ extends to a strong monad $\mathcal{L}^\meet$ on
meet-semilattices, and to a costrong comonad $\mathcal{L}^\join$ on bounded join-semilattices; the bottom
element of $X$ is needed to implement the counit $\varepsilon_X: \mathcal{L}^\join(X) \to X$. These two in
turn form a strong monad in $\LatGal$ with unit $\varepsilon^\join_X \dashv \eta^\meet_X$ and multiplication
$\delta^\join_X \dashv \mu^\meet_X$.
