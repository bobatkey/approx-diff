...
lists of natural numbers as $\mu\alpha.1 + \synVar{nat} \times \alpha$, one could then (within the language)
define a type of ``approximation lists'' $\mu\alpha.\synVar{L}\,(1 + \synVar{L}\,\synVar{nat} \times \alpha)$,
or perhaps a custom approximating version of multiplication $*_{\text{appr}}: \synVar{L}\,\synVar{nat} \times
\synVar{L}\,\synVar{nat} \to \synVar{L}\,\synVar{nat}$, expressible as
...
To interpret the syntactic type constructor $\synVar{L}$, \todo{show how $\LatGal$ has a lifting monad.}
