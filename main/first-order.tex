...
conventional semantics) must then also be interpreted by a pair $(f, \partial f)$, where $f$ provides the
conventional interpretation of $e$ as a function $X \to Y$, and $\partial f$ is a family of Galois connections
$\partial f(x): \partial X(x) \to \partial Y(f(x))$ for every $x \in X$, taking approximants to approximants.
