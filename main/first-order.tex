...
which acts on posets $X$ to extend them with a distinguished bottom element $\bot$ and which acts on monotone
functions $f$ between posets to extend them with a mapping from $\bot$ to $\bot$. $\mathcal{L}$ is also a
strong monad on meet-semilattices. On (bounded) join-semilattices, $\mathcal{L}$ induces a costrong comonad
$(\mathcal{L}, \varepsilon, \delta)$, with the bottom element of $X$ needed to implement the counit
$\varepsilon_X: \mathcal{L}(X) \to X$. These two combine to form a strong lifting monad in $\LatGal$ with unit
$\varepsilon_X \dashv \eta_X$ and multiplication $\delta_X \dashv \mu_X$.
