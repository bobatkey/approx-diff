...
interpret $\syn{nat}$ to the object $(\Nat,n \mapsto \downset{n})$, where the family component assigns to
...
lattice $\Two = \{\mathsf{tt},\mathsf{ff}\}$; using $\downset{n}$ is slightly more ergonomic as the
approximants of a given value then contain, rather than merely determining, all the information about the
value that hasn't been approximated away.

We might then interpret $+$ as the morphism $(+,\partial+)$, i.e.~the usual addition operator + paired with an
...
A programmer, or perhaps a language implementor, might also like to embed additional ``approximation points''
into composite structures, to allow for example the list $1 : 2 : []$ to have the partial list $1 : \bot$ as
an approximant, representing a list with at least one element. This can be accomplished by extending the
syntax with a \emph{lifting} type constructor $\synVar{L}$. Then, as an alternative to the usual definition of
lists of natural numbers as $\mu\alpha.1 + \synVar{nat} \times \alpha$, one could (within the language) define
a type of ``approximation lists'' $\mu\alpha.\synVar{L}\,(1 + \synVar{L}\,\synVar{nat} \times \alpha)$, with
...
\synVar{nat} \times \alpha$ would desugar automatically into the approximating version
$\mu\alpha.\synVar{L}\,(1 + \synVar{L}\,\synVar{nat} \times \alpha)$.

To interpret the syntactic type constructor $\synVar{L}$, consider the strong monad $(\mathcal{L}, \eta, \mu)$
which extends any poset $X$ with a distinguished bottom element $\bot$ and any monotone function $f$ between
posets with a mapping from $\bot$ to $\bot$. $\mathcal{L}$ is also a strong monad on meet-semilattices. On
(bounded) join-semilattices, $\mathcal{L}$ induces a costrong comonad $(\mathcal{L}, \varepsilon, \delta)$,
with the bottom element of $X$ needed to implement the counit $\varepsilon_X: \mathcal{L}(X) \to X$. These two
combine to form a strong lifting monad in $\LatGal$ with unit $\varepsilon_X \dashv \eta_X$ and multiplication
$\delta_X \dashv \mu_X$.

The lifting monad $\mathcal{L}$ in $\LatGal$ is preserved into $\Fam(\LatGal)$. The families construction,
regarded as a covariant 2-functor $\Fam(-): \Cat \to \Cat$, sends functors $F: \cat{C} \to \cat{D}$ to
functors $\Fam(F): \Fam(\cat{C}) \to \Fam(\cat{D})$ that reassign the target category by postcomposing with
$F$, sending objects $(X, \partial X)$ to $(X, F \comp \partial X)$ and morphisms $(f, \partial f)$ to $(f, F
\comp \partial f)$, where $F \comp \partial f$ denotes the natural transformation where $(F \comp \partial
f)(x) = F(\partial f(x))$. If $F$ happens to be a (strong) monad on $\cat{C}$, then $\Fam(F)$ is a (strong)
monad on $\Fam(\cat{C})$, computed by extending the original monad $F: \cat{C} \to \cat{C}$ componentwise to
families.
