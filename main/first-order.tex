...
which acts on a poset $X$ to extend it with a distinguished bottom element $\bot$ and which acts on a monotone
function $f$ between posets to extend it with a mapping from $\bot$ to $\bot$. $\mathcal{L}$ is also a strong
monad on meet-semilattices. On (bounded) join-semilattices, $\mathcal{L}$ induces a costrong comonad
