...
commutative monoid with an addition + and zero element 0. A category $\cat{C}$ is enriched in $\CMon$ (or a
$\CMon$-category), the category of commutative monoids, if every hom-set $\cat{C}(X,Y)$ has a similar additive
structure, providing an operation + to add morphisms together plus a zero morphism $0: X \to Y$, and if
