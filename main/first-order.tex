...
$1 : 2 : []$ now includes not only the ``shape-preserving'' approximations but also the prefixes obtained by
truncating successive tails with $\bot$:
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \node [left of=ioi,xshift=-3cm] (oii) {$\bot : 2 : []$};
      \node [right of=ioi,xshift=3cm] (iio) {$1 : 2 : \bot$};
      % row 3
      \node [below of=ioi] (oio) {$\bot : 2 : \bot$};
      \node [left of=oio,xshift=-3cm] (ooi) {$\bot : \bot : []$};
      \node [right of=oio,xshift=3cm] (ioo) {$1 : \bot : \bot$};
      % row 4
