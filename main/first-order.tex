...
definition of $\syn{list}\;\syn{nat}$ as $\mu\alpha.1 + \syn{nat} \times \alpha$, one could then define a type
of ``approximation lists'' $\mu\alpha.\syn{L}\;(1 + \syn{L}\;\syn{nat} \times \alpha)$, along with an
``approximating'' version of multiplication $*: \syn{L}\;\syn{nat} \times \syn{L}\;\syn{nat} \to
\syn{L}\;\syn{nat}$.
