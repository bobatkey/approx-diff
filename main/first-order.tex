...
To interpret the syntactic type constructor $\synVar{L}$, we note that $\LatGal$ has a lifting monad
$\mathcal{L}$ which sends any bounded lattice $X = (X, \meet, \join, \top, \bot)$ to the lattice
$\mathcal{L}(X) = X_\bot$.
