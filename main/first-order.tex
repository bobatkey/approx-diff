...
To say that a category $\cat{C}$ is enriched in $\CMon$, the (monoidal) category of commutative monoids, is to
say that every hom-set $\cat{C}(X,Y)$ provides an operation + to add morphisms together plus a zero morphism
$0: X \to Y$, and moreover that composition is \emph{bilinear}, i.e.~given by a family of morphisms
$\Hom{\cat{C}}{Y}{Z} \tensor \Hom{\cat{C}}{X}{Y} \to \Hom{\cat{C}}{X}{Z}$ in $\CMon$ that preserve the
additive structure in $\Hom{\cat{C}}{Y}{Z}$ and $\Hom{\cat{C}}{X}{Y}$ separately:
...
\begin{enumerate}
\item If $X \times Y$ is a product then it is also coproduct, with $\inj_X = \prodM{\id}{0}$ and $\inj_Y =
\prodM{0}{\id}$.
...
or fully absent ($\bot$) from the approximation, as shown in the example in \secref{introduction}, and we
...
the type of lists of natural numbers as $\mu\alpha.1 + \synVar{nat} \times \alpha$, one could (within the
language) define a type of ``approximation lists'' $\mu\alpha.\synVar{L}\,(1 + \synVar{L}\,\synVar{nat} \times
\alpha)$, along with an approximating version of multiplication $*_{\text{appr}}: \synVar{L}\,\synVar{nat}
\times \synVar{L}\,\synVar{nat} \to \synVar{L}\,\synVar{nat}$ expressible as
...
matrix convolutions, without baking this specific choice of approximation into the interpretation of
