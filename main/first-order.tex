...
\item
\begin{enumerate}
If $\cat{C}$ has (symmetric) monoidal products then $\Fam(\cat{C})$ has (symmetric) monoidal products
$(X, \partial X) \tensor (Y, \partial Y) = (X \times Y, \partial(X \times Y))$ indexed by the Cartesian
product $X \times Y$, where $\partial(X \times Y)(x,y) = \partial X(x) \tensor \partial Y(y)$.
\end{enumerate}
\end{proposition}
