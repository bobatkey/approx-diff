...
\begin{enumerate}
\item a strong monad $(\mathcal{L}, \eta, \mu)$ with strength $\mathsf{st}_{X,Y}: X \times \mathcal{L}(Y) \to
\mathcal{L}(X \times Y)$;
...
\item for any object $X$, an object $\List(X)$, morphisms $\nil: 1 \to \List(X)$ and $\cons: X \times
\List(X) \to \List(X)$, and for any object $Y$ and morphisms $1 \to Y$ and $X \times Y \to Y$, a morphism $Y
\times \List(X) \to Y$.
% x ⇒ z → prod (prod x y) z ⇒ z → prod x (list y) ⇒ z
\end{enumerate}
