...
\propref{ho-embedding}, because the functor $\Fam: \Cat \to \Cat$ preserves finite products and so does the
embedding. Finally $\LatGal$ has a lifting monad which is also preserved by $\Fam$
(\secref{first-order:lifting}). The correctness of this interpretation is considered in the next section.
