...
in $\Sem$. \figref{semantics:types} gives the interpretations of types $\tau$ and contexts $\Gamma$ as objects
of $\Sem$. \figref{semantics:terms} gives the interpretation of terms $\Gamma \vdash t: \tau$ as morphisms
$\sem{\Gamma} \to \sem{\tau}$.
...
implement $\Sem$. It is bicartesian closed by \corref{mslat-jslat-bcc}, and can model $\List(X)$ as an
infinite coproduct (\secref{models-of-total-gps:coproducts-and-products}). A model $H(M)$ of $\Sigma$ can be
obtained via the embedding $\HoEmbed: \Fam(\LatGal) \to \Fam(\MeetSLat \times \JoinSLat^\op)$ given in
\propref{ho-embedding}, because .... Finally $\LatGal$ has a lifting monad which is preserved by the 2-functor
$\Fam: \Cat \to \Cat$ (\secref{first-order:lifting}). The correctness of this interpretation is considered in
the next section.
