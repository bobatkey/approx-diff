...
$\LatGal$ is not Cartesian closed, and so cannot serve directly as the basis for interpreting higher-order
...
\Rightarrow Y} \iso 1$, and $\cat{C}$ has only a single object, up to isomorphism. However although biproducts
rule out Cartesian closure, they will prove crucial for establishing a Cartesian closed setting in
\secref{higher-order}; but let us first see how to develop a suitable categorical setting for first-order
programs.
