...
\todo{Introduce that $\LatGal$ is $\SemiLat$-enriched -- the hom-set $\LatGal(X)(Y)$ is has an (idempotent) addition of morphisms and a zero morphism. Moreover it has biproducts -- product of lattices $X \times Y$ is also a coproduct, although formal definition will come later.}
...
We will consider a pure, total language, where types can be interpreted as sets. Our goal is to interpret
