...
$\reindex{F}{f}$ where $\reindex{F}{f}(x) = F(f(x))$ for any $x \in X$, and which sends any morphism $\eta: F
\to G$ between $Y$-indexed families to the morphism $\reindex{g}{f}: \reindex{X}{f} \to \reindex{Y}{f}$
between $X$-indexed families, with $\reindex{\eta}{f}(x) = \eta(f(x))$.
\end{definition}
