...
pointwise lifting of $\meet$ to meet-semilattice homomorphisms $f, g: X \to Y$, where $(f \meet g)(x) = f(x)
\meet g(x): X \to Y$, gives a notion of ``addition'' for these maps which is associative and commutative, and
which has a ``zero'' homomorphism $0_{X,Y}: X \to Y$ given by the constant map sending any $x \in X$ to $0_Y$,
acting as a unit for addition of homomorphisms. The same can be said for the join-preserving maps from $X$ to
$Y$, which also have a commutative monoid structure; Galois connections inherit this structure component-wise.
We now say more generally how this so-called \emph{semi-additive} structure on morphisms arises and explain
its significance to adapting CHAD to provide a semantic basis for Galois slicing as well as automatic
differentiation.
