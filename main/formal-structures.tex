...
unit for $\meet$. Dually the (bounded) join-preserving maps from $Y$ to $X$ have a join-semilattice structure
given pointwise by $\join$ and the constant homomorphism $\bot_{Y,X}: Y \to X$ sending any $y \in Y$ to
$\bot_X$. Since these two constructions are adjoint, $\LatGal$ is enriched in the category $\SemiLat$ of
