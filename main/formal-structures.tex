...
posets, where each is the closest approximation (one from above, one from below) to an inverse of the other.
...
adjunction between poset categories, with monotone $f$ and $g$ interpreted as functors; $f$ is usually
...
As shown in \secref{introduction:galois-slicing}, for Galois slicing we require a setting where the
...
\begin{definition}
Define the category $\LatGal$ which has as objects $X, Y$ all bounded lattices, and as morphisms all Galois
connections $f \adj g: X \to Y$.
\end{definition}
