...
interpreting forward and backward Galois slicing. Galois connections are pairs of monotone functions between
posets, where each is the closest approximation to an inverse of the other: one from above, one from below.

\begin{definition}[Galois connection]
Suppose $X$ and $Y$ are posets. A \emph{Galois connection} $f \adj g: X \to Y$ is a pair of monotone functions
$f: X \to Y$ and $g: Y \to X$ satisfying $y \leq f(x) \iff g(y) \leq x$ for any $x \in X$ and $y \in Y$.
\end{definition}

\noindent The $\adj$ notation is justified because a Galois connection $f \adj g: X \to Y$ can also be seen an
adjunction between poset-as-categories, with monotone $f$ and $g$ interpreted as functors; $f$ is usually
referred to as the \emph{upper} (right) adjoint and $g$ as the \emph{lower} (left) adjoint. Galois connections
compose component-wise, with $\id_X \adj \id_X: X \to X$ as the unit for composition, and thus form a category
where the objects are posets and the morphisms are Galois connections.

As indicated in \secref{introduction:galois-slicing}, for Galois slicing we require a setting where the
approximants of a point form a bounded lattice, with least element $\bot$ representing the approximation that
discards all information and greatest element $\top$ representing the approximation that discards no
information; so we define the following subcategory.

\begin{definition}
Define the category $\LatGal$ which has as objects $X, Y$ all bounded lattices, and as morphisms from $X$ to
$Y$, all Galois connections $f \adj g: X \to Y$.
\end{definition}
...
The unit $0$ is both terminal and initial and is called a \emph{zero} object. A semi-additive category
$\cat{C}$ is enriched in $\CMon$, the category of commutative monoids: for any two morphisms $f, g: X \to Y$
in $\cat{C}$, the biproduct structure provides a way to ``add'' them together, forming a morphism $f + g: X
\to Y$. Diagrammatically:
...
It is easy to verify that $+$ is associative and commutative and that $f + 0 = f = 0 + f$, and thus that every
