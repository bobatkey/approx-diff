...
For a total language where types denote sets, the goal is to interpret programs as Galois connections relating
approximations of their input to approximations of their output. Since these lattice of approximations depends
on the value being approximated, the semantics of every type $\tau$ must have both a ``conventional''
...
gloms all such $\Set$-indexed families together, with reindexing morphisms to transport between different
indexing sets.
