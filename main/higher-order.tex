...
Suppose $\cat{C}$ a Cartesian closed category with exponentials $\internalHom{X}{Y}$, evaluation morphisms
$\eval_{X,Y}$ and currying isomorphisms $\lambda_{X,Y,Z}$, and the following additional structure:
\begin{enumerate}
...
\end{enumerate}
\figref{default-semantics:types} gives the interpretation of a type $\Delta \vdash \tau$ with $\length{\Delta}
= n$ as a multifunctor $\cat{C}^n \to \cat{C}$ (as an object of $\cat{C}$ in the case $n = 0$).
