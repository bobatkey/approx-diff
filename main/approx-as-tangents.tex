...
\bob{mention chain rule here? and tangent spaces?}
...
\begin{proof}
  ...
  the stable ordering $f \sqsubseteq g$ iff \bob{FIXME}.
\end{proof}
...
In light of the \exref{non-stable-function}, we consider
...
We have two reasons for moving to a slightly different model in \secref{model}:
...
\item Even without completeness, in Bounded meet posets and L-posets,
  ...
  to be the same thing. \bob{fwd reference to discussion.}) In
  \exref{intervals-and-maxima-elements}, maximal elements could be
...
\bob{For future work: what is the connection between FamCPO(LatGal)
  and Laird's bistable biorders, or Berry bidomains?}
...
refined model of partially ordered approximations in
\secref{model-summary}.
