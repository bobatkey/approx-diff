...
of a composite function without have to recompute the function itself, so long as all atomic operations
provide derivatives. An efficient implementation of \GPS based on $\Fam(\LatGal)$ (\secref{fam:galois-slicing}
above) would most likely apply a similar optimisation to avoid recomputing $f(x)$ at each step during forward
slicing.
