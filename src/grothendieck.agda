...
module CategoryOfFamilies {o m e} os (𝒞 : Category o m e) where

  open Fam

  record Obj : Set (o ⊔ suc m ⊔ suc e ⊔ suc os) where
    no-eta-equality
    field
      idx : Setoid (m ⊔ e ⊔ os) (m ⊔ e ⊔ os)
      fam : Fam idx 𝒞
  open Obj

  record Mor (X Y : Obj) : Set (os ⊔ m ⊔ e) where
    no-eta-equality
    field
      idxf : X .idx ⇒s Y .idx
  ...
  record _≃_ {X Y : Obj} (f g : Mor X Y) : Prop (os ⊔ m ⊔ e) where
    no-eta-equality
    field
  ...
  module _ where
    ...
    cat : Category (o ⊔ suc m ⊔ suc e ⊔ suc os) (os ⊔ m ⊔ e) (e ⊔ m ⊔ os)
    cat .obj = Obj
    cat ._⇒_ = Mor
    cat ._≈_ = _≃_
...
-}
  ...
  -- is cartesian closed.
  module _ (CM𝒞 : CMonEnriched 𝒞)
           (BP : ∀ x y → Biproduct CM𝒞 x y)
           (SP : HasSetoidProducts (m ⊔ e ⊔ os) (m ⊔ e ⊔ os) 𝒞) where
    ...
    nudge : ∀ {X Y : Setoid (m ⊔ e ⊔ os) (m ⊔ e ⊔ os)} → X .Carrier → Y ⇒s ⊗-setoid X Y
    nudge x .func y = x , y
    nudge {X} x .func-resp-≈ e = X .refl , e

    nudge-≈ : ∀ {X Y : Setoid (m ⊔ e ⊔ os) (m ⊔ e ⊔ os)} {x₁ x₂} → X ._≈_ x₁ x₂ → nudge {X = X} {Y = Y} x₁ ≈s nudge x₂
