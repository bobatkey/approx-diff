...
\cat{C} \to \Func{\cat{C}^\op}{\Set}$ sending:
\begin{itemize}
\item any object $X$ in $\cat{C}$ to the contravariant hom-functor $\Yoneda(X) = \Hom{\cat{C}}{-}{X}:
\cat{C}^\op \to \Set$
which sends:
   \begin{itemize}
   ...
   \end{itemize}
\item any morphism $f: X \to Y$ in $\cat{C}$ to the natural transformation $\Yoneda(f)$ where $\Yoneda(f)_Z =
(f \comp -): \Hom{\cat{C}}{Z}{X} \to \Hom{\cat{C}}{Z}{Y}$.
\end{itemize}

\noindent Now suppose $\cat{C}$ is $\mathscr{V}$-enriched with $\mathscr{V}$ closed monoidal; then the
hom-functor $\Hom{\cat{C}}{-}{X}: \cat{C}^\op \to \mathscr{V}$ is $\mathscr{V}$-enriched. The following
