...
\cat{C} \to \Func{\cat{C}^\op}{\mathscr{V}}_{\mathscr{V}}$ sending objects $X$ to the enriched hom-functor
$\Yoneda_{\mathscr{V}}(X) = \Hom{\cat{C}}{-}{X}: \cat{C}^\op \to \mathscr{V}$ and providing a family of
morphisms $(\Yoneda_{\mathscr{V}})_{X,Y}: \Hom{\cat{C}}{X}{Y} \to
\Hom{\Func{\cat{C}^\op}{\mathscr{V}}_{\mathscr{V}}}{\Hom{\cat{C}}{-}{X}}{\Hom{\cat{C}}{-}{Y}}$.

The enriched Yoneda lemma says that for any $\mathscr{V}$-enriched presheaf $F$ and any object $X \in \cat{C}$
the hom-object of natural transformations between the $\mathscr{V}$-enriched presheaf $\Hom{\cat{C}}{-}{X}$
and $F$, which also lives in $\mathscr{V}$, is isomorphic to $F(X)$:
